...
public class RevisedGuiWindow : MonoBehaviour 
{
	...
	void Start()
	{
		planeObj = GameObject.Find("Ground");
	}
	...
	private void ShowGuiWindow(int windowID) 
	{
		...
		GUILayout.Space(30);
		isPlaneOn = GUILayout.Toggle(isPlaneOn, "Enable Ground");
		SetPlaneVisible(isPlaneOn);

		GUILayout.Space(30);
		isGravityOn = GUILayout.Toggle(isGravityOn, "Enable Gravity");
		SetGravity(isGravityOn);
		
		GUILayout.Space(30);
		isControlMouseOn = GUILayout.Toggle(isControlMouseOn, "Enable Mouse Control");
