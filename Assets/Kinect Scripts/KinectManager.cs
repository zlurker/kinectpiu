...
//		}
	...
	public string GetWorldMatrixData()
	{
		return KinectInterop.GetMatrixAsCsv(ref kinectToWorld);
	}
	...
	public string GetBodyHandData(ref long liRelTime)
	{
		return KinectInterop.GetHandsDataAsCsv(sensorData, ref bodyFrame, ref liRelTime);
	}
               ...
               bodyData.joint[parent].trackingState != KinectInterop.TrackingState.NotTracked) {
                ...
                if (posParent != Vector2.zero && posJoint != Vector2.zero) {
                    ...
                    if (i == 15 && legs[0] != null) { //The two joints I need. Use these two to create feet.
                        ...
                        legs[0].position = new Vector3(legs[0].position.x*7.5f,0, 0);
                    }
                    if (i == 19 && legs[1] != null) {
                        KinectInterop.DrawLine(aTexture, (int)posParent.x, (int)posParent.y, (int)posJoint.x, (int)posJoint.y, Color.yellow);
                        //Debug.Log(i + " " + bodyData.joint[i].kinectPos);
                        legs[1].position = bodyData.joint[i].kinectPos;
                        legs[1].position = new Vector3(legs[1].position.x*7.5f,0,0);
                    }
